...
def validate_model(model: BuildingModel) -> List[ValidationIssue]:
    ...
    valid_layer_ids = {l.id for l in model.layers}
    valid_level_ids = {l.id for l in model.levels}
    ...
    for m in model.materials: check_id(m.id)
    for l in model.layers: check_id(l.id)
    for lvl in model.levels: check_id(lvl.id)
    ...
    for level in model.levels:
        ...
        for wall in level.walls:
            ...
            if wall.level_id != level.id:
                issues.append(ValidationIssue(wall.id, f"Wall level_id '{wall.level_id}' does not match its parent level '{level.id}'."))
            if wall.material_id_exterior and wall.material_id_exterior not in valid_material_ids:
                issues.append(ValidationIssue(wall.id, f"Invalid material_id_exterior: {wall.material_id_exterior}"))
            if wall.material_id_interior and wall.material_id_interior not in valid_material_ids:
                issues.append(ValidationIssue(wall.id, f"Invalid material_id_interior: {wall.material_id_interior}"))
            ...
            if len(wall.centerline) == 2:
                ...
                if length < 1.0:
                    issues.append(ValidationIssue(wall.id, "Wall centerline length is less than 1mm."))
            else:
                issues.append(ValidationIssue(wall.id, "Wall centerline must have exactly 2 points."))

        # Validate Openings
        for op in level.openings:
            check_id(op.id)
            if op.wall_id not in valid_wall_ids:
                issues.append(ValidationIssue(op.id, f"Opening references non-existent wall_id: {op.wall_id}"))
            ...
            if wall:
                ...
                if op.offset_mm + op.width_mm > length:
                    issues.append(ValidationIssue(op.id, f"Opening offset ({op.offset_mm}) + width ({op.width_mm}) exceeds wall length ({length:.2f})."))
                if op.sill_mm + op.height_mm > wall.height_mm:
                    issues.append(ValidationIssue(op.id, f"Opening sill ({op.sill_mm}) + height ({op.height_mm}) exceeds wall height ({wall.height_mm})."))
