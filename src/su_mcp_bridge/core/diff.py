...
_LEVEL_NESTED_FIELDS = frozenset({
    "walls", "openings", "slabs", "roofs", "columns",
    "beams", "components", "dimensions",
})
...
def _is_nested_entity_list(field_name: str, entity_type: str, value: Any, old_value: Any) -> bool:
    ...
    for candidate in (value, old_value):
        if isinstance(candidate, list) and candidate and isinstance(candidate[0], dict) and "id" in candidate[0]:
            return True
    return False


def _diff_entities(old_entities: List[Any], new_entities: List[Any], entity_type: str, changeset: ChangeSet):
    ...
    for eid, new_e in new_dict.items():
        if eid not in old_dict:
            changeset.created.append(EntityCreated(
                entity_id=eid,
                entity_type=entity_type,
                data=new_e.model_dump()
            ))
        else:
            old_e = old_dict[eid]
            ...
            if changes:
                changeset.modified.append(EntityModified(
                    entity_id=eid,
                    entity_type=entity_type,
                    changes=changes
                ))
    ...
    for eid in old_dict:
        if eid not in new_dict:
            changeset.deleted.append(EntityDeleted(
                entity_id=eid,
                entity_type=entity_type
            ))
...
def diff_models(old_model: BuildingModel, new_model: BuildingModel) -> ChangeSet:
    ...
    old_walls = [w for l in old_model.levels for w in l.walls]
    new_walls = [w for l in new_model.levels for w in l.walls]
    _diff_entities(old_walls, new_walls, "Wall", cs)
    
    old_openings = [o for l in old_model.levels for o in l.openings]
    new_openings = [o for l in new_model.levels for o in l.openings]
    _diff_entities(old_openings, new_openings, "Opening", cs)
    
    old_slabs = [s for l in old_model.levels for s in l.slabs]
    new_slabs = [s for l in new_model.levels for s in l.slabs]
