...
class HouseGenerator:
    ...
    def build(self, config: dict) -> dict:
        ...
        cfg = HouseConfig(**{k: v for k, v in config.items()
                             if k in HouseConfig.__dataclass_fields__})

        print(f"\n{'='*60}")
        print(f"  [HOUSE] House Generator v3")
        print(f"  {cfg.bedrooms} bedrooms, {cfg.bathrooms} bath"
    ...
    def _compute_layout(self, cfg: HouseConfig) -> dict:
        ...
        if cfg.bedrooms == 1:
            ...
            back_rooms = [("bed1", bed_w), ("bed2", bed_w),
                           ("bed3", bed_w), ("bath", bath_w)]
        else:
            back_rooms = [("bed1", bed_w), ("bath", bath_w)]
        ...
        kitchen_w = interior_w - living_w - it  # minus one partition

        # OUTER dimensions of the house
        W = interior_w + 2 * t   # two exterior side walls
        D = front_d + back_d + it + 2 * t   # front wall + back wall + horiz partition

        # Y coordinate of horizontal partition centerline
        partition_y = t + front_d + it / 2.0

        # X positions of back room partitions (centerlines, from left)
        back_partition_xs = []
        x = t  # start from inner face of left wall
        for i, (name, rw) in enumerate(back_rooms[:-1]):
            x += rw
        ...
        for i, (name, rw) in enumerate(back_rooms):
            room_x_starts.append(x)
            x += rw + it

        # Living/kitchen partition X
        lk_partition_x = t + living_w + it / 2.0
    ...
    def _generate(self, cfg):
        ...
        # Slabs
        # Plot Base (Lawn)
        self._do("ops.slab.create", "SLAB_LAWN", {
            "polygon": [[0, 0, -200], [PW, 0, -200], [PW, PD, -200], [0, PD, -200]],
            "thickness_mm": 200, "base_z_mm": -200
        })
        # House Plinth
        self._do("ops.slab.create", "SLAB_HOUSE", {
            "polygon": [[HX, HY, 0], [HX+HW, HY, 0], [HX+HW, HY+HD, 0], [HX, HY+HD, 0]],
            "thickness_mm": 200, "base_z_mm": -200
        })
        # Car Porch
        self._do("ops.slab.create", "SLAB_PORCH", {
            "polygon": [[2000, t, -50], [5600, t, -50], [5600, HY, -50], [2000, HY, -50]],
            "thickness_mm": 150, "base_z_mm": -150
        })
        # Walkway Path
        self._do("ops.slab.create", "SLAB_PATH", {
        ...
        # Roof
        overhang = 500
        self._do("ops.roof.create", "ROOF_MAIN", {
            "footprint": [
    ...
    def _wall(self, aid, centerline, thickness, height):
        ...
        self.client.send_request("ops.wall.create", {
            "ai_id": aid,
    ...
    def _cut(self, aid, wall_id, offset, width, height, sill):
        ...
        try:
            self.client.send_request("ops.opening.cut", {
                "ai_id": aid,
                "wall_id": wall_id,
                "offset_mm": offset,
                "width_mm": width,
    ...
    def _place_door(self, opening_id, x, y, width, height, wall_thick, rotation=0):
        ...
        try:
            self.client.send_request("ops.component.place", {
                "recipe": "door",
                "position_mm": [x, y, 0],
                "width_mm": width,
                "height_mm": height,
    ...
    def _apply_materials(self, cfg):
        palettes = {
            "modern":  {"MAT_WALL": "E8E0D8", "MAT_ROOF": "3D3D3D", "MAT_FLOOR": "C4B8A8", "MAT_LAWN": "4CAF50", "MAT_PORCH": "9E9E9E", "MAT_PATH": "795548"},
            "classic": {"MAT_WALL": "D4A574", "MAT_ROOF": "8B4513", "MAT_FLOOR": "A9A9A9", "MAT_LAWN": "4CAF50", "MAT_PORCH": "9E9E9E", "MAT_PATH": "795548"},
            "minimal": {"MAT_WALL": "FFFFFF", "MAT_ROOF": "2C2C2C", "MAT_FLOOR": "DCDCDC", "MAT_LAWN": "4CAF50", "MAT_PORCH": "9E9E9E", "MAT_PATH": "795548"},
        }
        colors = palettes.get(cfg.style, palettes["modern"])

        for mat_id, hex_color in colors.items():
            try:
                self.client.send_request("ops.material.upsert",
                                         {"id": mat_id, "color_hex": hex_color})
            except Exception:
                pass
        ...
        if wall_ids:
            try:
                self.client.send_request("ops.material.assign",
                                         {"material_id": "MAT_WALL", "target_ids": wall_ids})
            except Exception:
                pass

        try:
            self.client.send_request("ops.material.assign", {"material_id": "MAT_ROOF", "target_ids": ["ROOF_MAIN"]})
            self.client.send_request("ops.material.assign", {"material_id": "MAT_LAWN", "target_ids": ["SLAB_LAWN"]})
            self.client.send_request("ops.material.assign", {"material_id": "MAT_PORCH", "target_ids": ["SLAB_PORCH"]})
            self.client.send_request("ops.material.assign", {"material_id": "MAT_PATH", "target_ids": ["SLAB_PATH"]})
            self.client.send_request("ops.material.assign", {"material_id": "MAT_FLOOR", "target_ids": ["SLAB_HOUSE"]})
        except Exception:
            pass
